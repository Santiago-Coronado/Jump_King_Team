...
class Player extends AnimatedObject {
    constructor(_color, width, height, x, y, _type, physics) {
        ...
        // Movement variables
        this.movement = {
            right: { status: false, axis: "x", sign: 1, repeat: true, duration: 40, moveFrames: [1, 2, 3], idleFrames: [0] },
            left:  { status: false, axis: "x", sign: -1, repeat: true, duration: 40, moveFrames: [14, 15, 16], idleFrames: [17] },
            jump:  { status: false, repeat: false, duration: 100, right: [5, 6], left: [11, 11] },
            crouch:{ status: false, repeat: false, duration: 300, right: [4,4], left: [4,4], upRight: [0, 0], upLeft: [17, 17] },
        };
    }
    ...
    */
    ...
    jump() {
        if (!this.isJumping) {
            ...
            this.setAnimation(Math.min(...jumpFrames), Math.max(...jumpFrames), jumpData.repeat, jumpData.duration);
    ...
    dash() {
        if (this.powerUps.dash && !this.isDashing) {
            if (this.isJumping && this.hasAirDashed) return;
            const dashSpeed = this.physics.walkSpeed * 3;
            this.isDashing = true;
            const dashTime = 250;

            if (this.isJumping) this.hasAirDashed = true;
            this.velocity.y = 0;
            this.velocity.x = this.isFacingRight ? dashSpeed : -dashSpeed;
    ...
    crouch() {
        if (this.powerUps.charged && !this.isJumping) {
            this.isCrouching = true;
            const crouchData = this.movement.crouch;
            const crouchFrames = this.isFacingRight ? crouchData.right : crouchData.left;
            this.setAnimation(Math.min(...crouchFrames), Math.max(...crouchFrames), crouchData.repeat, crouchData.duration);
            this.velocity.x = 0;
        }
    }
    standUp(){
